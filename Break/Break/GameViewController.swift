...
//        {
//        
//        // if we didn't have the self.animator ??, we would be creating a new object everytime we called animator.addBehavior. Now we are using the same object.
//        
...
//    }
    ...
    override func viewDidLoad() {
        ...
        let brickWidth = (Int(view.frame.width) - totalSpacing) / cols
        
        for c in 0..<cols {
            
            for r in 0..<rows {
                
                let x = c * (brickWidth + brickSpacing) + brickSpacing
                let y = r * (brickHeight + brickSpacing) + brickSpacing + 60
                
                let brick = UIView(frame: CGRect(x: x, y: y, width: brickWidth, height: brickHeight))
    ...
    } // end when item and boundary collision
